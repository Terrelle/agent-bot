...
def _resolve_conversation_log(session_key: Optional[str] = None) -> "ConversationLog":
    from ..log import get_conversation_log

    return get_conversation_log(session_key=session_key)
...
async def summarize_conversation(session_key: Optional[str] = None) -> bool:
    settings = get_settings()
    if not settings.summarization_enabled:
        return False

    conversation_log = _resolve_conversation_log(session_key=session_key)
    working_memory_log = get_working_memory_log(session_key=session_key)
