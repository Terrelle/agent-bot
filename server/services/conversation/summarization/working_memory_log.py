...
def _working_memory_path_for_session(session_key: str) -> Path:
    normalized = normalize_session_key(session_key)
    if normalized == "default":
        return _WORKING_MEMORY_LOG_PATH
    slug = _session_to_slug(normalized)
    return _DATA_DIR / "conversation" / "sessions" / f"working_memory_{slug}.log"
...
_working_memory_logs: dict[str, WorkingMemoryLog] = {}
_factory_lock = threading.Lock()


def get_working_memory_log(session_key: Optional[str] = None) -> WorkingMemoryLog:
    resolved = normalize_session_key(session_key or get_active_session_key())
    with _factory_lock:
        existing = _working_memory_logs.get(resolved)
        if existing is not None:
            return existing

        created = WorkingMemoryLog(_working_memory_path_for_session(resolved))
        _working_memory_logs[resolved] = created
        return created
