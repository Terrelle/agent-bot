    ...
    async def _run_interaction() -> None:
        try:
            ...
            if not result.success:
                logger.error("chat task failed", extra={"error": result.error or "unknown error"})
                runtime.conversation_log.record_reply(
                    "Sorry, I ran into an internal error while processing that message."
                )
