...
def _session_to_slug(session_key: str) -> str:
    candidate = re.sub(r"[^A-Za-z0-9_.-]+", "_", session_key).strip("._")
    if not candidate:
        candidate = "default"
    if len(candidate) > 64:
        digest = sha1(session_key.encode("utf-8")).hexdigest()[:10]
        candidate = f"{candidate[:48]}_{digest}"
    return candidate


def _conversation_path_for_session(session_key: str) -> Path:
    normalized = normalize_session_key(session_key)
    if normalized == "default":
        return _CONVERSATION_LOG_PATH
    slug = _session_to_slug(normalized)
    return _DATA_DIR / "conversation" / "sessions" / f"conversation_{slug}.log"
...
def _resolve_working_memory_log(session_key: str) -> "WorkingMemoryLog":
    from .summarization import get_working_memory_log

    return get_working_memory_log(session_key=session_key)
...
class ConversationLog:
    ...
    def __init__(self, path: Path, session_key: str, formatter: TranscriptFormatter = _default_formatter):
        self._path = path
        self._session_key = normalize_session_key(session_key)
        self._formatter = formatter
        self._lock = threading.Lock()
        self._ensure_directory()
        self._working_memory_log = _resolve_working_memory_log(self._session_key)
    ...
    def record_reply(self, content: str) -> None:
        timestamp = self._append("genzbuzz_reply", content)
        self._working_memory_log.append_entry("genzbuzz_reply", content, timestamp)
    ...
    def _notify_summarization(self) -> None:
        ...
        try:
            schedule_summarization(session_key=self._session_key)
        except Exception as exc:  # pragma: no cover - defensive
            logger.warning(
...
def _resolve_session_key(session_key: Optional[str]) -> str:
    ...
    return get_active_session_key()


def get_conversation_log(session_key: Optional[str] = None) -> ConversationLog:
    resolved = _resolve_session_key(session_key)
    with _conversation_logs_lock:
        existing = _conversation_logs.get(resolved)
        if existing is not None:
            return existing

        created = ConversationLog(
            _conversation_path_for_session(resolved),
            session_key=resolved,
        )
        _conversation_logs[resolved] = created
        return created
