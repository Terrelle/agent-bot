...
# Initialize background services (trigger scheduler, retry scheduler, and email watcher) when the app starts
async def _start_trigger_scheduler() -> None:
    scheduler = get_trigger_scheduler()
    await scheduler.start()
    retry_scheduler = get_genzbuzz_retry_scheduler()
    await retry_scheduler.start()
    watcher = get_important_email_watcher()
    await watcher.start()
    memento_watcher = get_memento_watcher()
    await memento_watcher.start()
