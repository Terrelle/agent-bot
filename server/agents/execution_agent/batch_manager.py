...
class ExecutionBatchManager:
    ...
    def __init__(self, timeout_seconds: int = 90) -> None:
        ...
        self._batch_state: Dict[str, _BatchState] = {}
    ...
    async def execute_agent(
        ...
        await self._complete_execution(batch_id, result, agent_name, session_key)
        return result
    ...
    async def _register_pending_execution(
        ...
        async with self._batch_lock:
            state = self._batch_state.get(session_key)
            if state is None:
                batch_id = str(uuid.uuid4())
                state = _BatchState(batch_id=batch_id)
                self._batch_state[session_key] = state
            else:
    ...
    async def _complete_execution(
        ...
        async with self._batch_lock:
            ...
            if state.pending == 0:
                ...
                self._batch_state.pop(session_key, None)

        if dispatch_payload:
            await self._dispatch_to_interaction_agent(dispatch_payload, session_key)
    ...
    async def _dispatch_to_interaction_agent(self, payload: str, session_key: str) -> None:
        ...
        runtime = InteractionAgentRuntime(session_key=session_key)
        try:
