...
def _render_conversation_history(transcript: str) -> str:
    history = _truncate_history(transcript.strip())
    if not history:
        history = "None"
    return f"<conversation_history>\n{history}\n</conversation_history>"


def _truncate_history(history: str) -> str:
    if len(history) <= MAX_HISTORY_CHARS:
        return history

    # Keep the latest context where most current intent and tool outcomes live.
    suffix = history[-MAX_HISTORY_CHARS:]
    return f"[conversation_history_truncated]\n{suffix}"
