...
DEFAULT_APP_NAME = "GenZbuzz Agent Server"
...
class Settings(BaseModel):
    ...
    interaction_agent_model: str = Field(
        default=os.getenv("OPENPOKE_INTERACTION_AGENT_MODEL", "meta-llama/llama-3.1-8b-instruct")
    )
    execution_agent_model: str = Field(
        default=os.getenv("OPENPOKE_EXECUTION_AGENT_MODEL", "meta-llama/llama-3.1-8b-instruct")
    )
    execution_agent_search_model: str = Field(
        default=os.getenv("OPENPOKE_EXECUTION_AGENT_SEARCH_MODEL", "meta-llama/llama-3.1-8b-instruct")
    )
    summarizer_model: str = Field(
        default=os.getenv("OPENPOKE_SUMMARIZER_MODEL", "meta-llama/llama-3.1-8b-instruct")
    )
    email_classifier_model: str = Field(
        default=os.getenv("OPENPOKE_EMAIL_CLASSIFIER_MODEL", "meta-llama/llama-3.1-8b-instruct")
    )

    # Credentials / integrations
    openrouter_api_key: Optional[str] = Field(default=os.getenv("OPENROUTER_API_KEY"))
    openrouter_max_tokens: int = Field(default=_env_int("OPENROUTER_MAX_TOKENS", 512))
    ...
    # GenZbuzz WordPress bridge
    genzbuzz_wp_ajax_url: str = Field(
        default=os.getenv(
            "OPENPOKE_WP_AJAX_URL",
            "",
        )
    )
    genzbuzz_wp_referer: str = Field(
        default=os.getenv("OPENPOKE_WP_REFERER", "")
    )
    genzbuzz_wp_timeout_seconds: int = Field(
        default=_env_int("OPENPOKE_WP_TIMEOUT_SECONDS", 30)
    )
    genzbuzz_retry_poll_interval_seconds: int = Field(
        default=_env_int("OPENPOKE_GENZBUZZ_RETRY_POLL_INTERVAL_SECONDS", 30)
    )
    genzbuzz_retry_batch_limit: int = Field(
        default=_env_int("OPENPOKE_GENZBUZZ_RETRY_BATCH_LIMIT", 20)
    )
    genzbuzz_memento_watcher_poll_interval_seconds: int = Field(
        default=_env_int("OPENPOKE_GENZBUZZ_MEMENTO_WATCHER_POLL_INTERVAL_SECONDS", 75)
    )
    genzbuzz_memento_watcher_lookback_minutes: int = Field(
        default=_env_int("OPENPOKE_GENZBUZZ_MEMENTO_WATCHER_LOOKBACK_MINUTES", 30)
    )
